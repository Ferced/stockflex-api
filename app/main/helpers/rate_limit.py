...
class RateLimit:
    ...
    def is_ip_allowed(ip):
        ...
        return RateLimit.request_is_not_limited(Connections.redis_client, ip, limit, period)

    @staticmethod
    def is_path_allowed(path):
        limit = LimiterConstants.path_limit
        period = timedelta(seconds=LimiterConstants.path_limit_period)
        return RateLimit.request_is_not_limited(Connections.redis_client, path, limit, period)
    ...
    def is_combo_allowed(key1, key2):
        ...
        key = "{%s:%s}" % (key1, key2)
        return RateLimit.request_is_not_limited(Connections.redis_client, key, limit, period)
